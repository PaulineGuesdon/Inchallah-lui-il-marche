...
of $170~\text{A}$. This trap is crucial for two reasons: it enables to accumulate atoms the more atoms in the metastable 
...
$^3P_2 \rightarrow {}^3D_2$ transition, to recover approximately 1:4 of the atoms that decayed
...
$\simeq 1~\mu\text{mm}$. The second stage, at the same gradient, further cools
...
further evaporation to $\simeq$100~nK in a round surface of about 10~$\mu$m diameter.
In Chapter 3, the atoms are held in one or two arms of the ODT only,
within a round region of $\simeq 100~\mu\text{m}$, reaching $T \simeq 30~\mu\text{K}$
...
%N=10000 et 1 spin pop, n = 5*10^13atm/cm^3 en début d'évap
...
each stage's description below.
...
\begin{sidewaysfigure}
    ...
    are the total intensity shared in the different fibers concerned. \textbf{Top:} magnetic field gradient (BMOT) and the three bias/compensation coil voltages (VA, VC, VD). \textbf{Middle:} intensities of the $461~$nm, $403~$nm, $689~$nm, and $1075~$nm light. \textbf{Bottom:} frequency detunings of the MOT and STIR (stirring) beams during the broadband/SWAP and narrowband stages, and of the reservoir/dimple beams during evaporation. The bottom right inset details the imaging sequence (probe, reference, and dark exposures).}
...
\begin{equation}
    \frac{dN(t)}{dt} = \alpha \Gamma -\gamma_{loss}N(t)
\end{equation}
...
\begin{equation}
    N(t) = N_{sat}(1-e^{-t/\tau})
\end{equation}
$N_{sat} = \alpha \Gamma \tau$ when the transition reaches its maximum value. To saturate the transition one should wait for few blue MOT lifetime which corresponds to 100~ms to 1~sec. \\

The experimental duration is set differently since one must consider the time for which one reaches the maximum atom number trapped in the metastable state: this corresponds to 8.2~sec. This waiting time is 
to accumulate all the atoms in the metastable state since at a photon absorption 3:4 of them are still in the blue MOT cycle. Meanwhile the magnetic trap is on to keep the atoms in the metastable state.
From this accumulation the atoms are repumped (see subsection down below) to $^3D_2$ state and then decay to the $^3P_1$ state where they go back to the ground state. There are then adressed by the lasers one turn on before 
the repumping laser so during the short process of repumping, all the atoms in the ground state are trapped by the next red MOT stage.


\subsection{Repumper}

After the accumulation of all the atoms in the metastable state, a repumping laser system is required: approximately $1:4$ of the 1:20000 atoms that decayed from $^1P_1$ to $^1D_2$ into the metastable state (lifetime $\tau 
\approx 500~\text{s}$), the rest in $^3P_1$. Without repumping, atoms become dark to the $461~\text{nm}$ light and are lost from the cooling cycle. 
...
For this transition, the formal Doppler limit ($T_D = \hbar \Gamma / 2 k_B = 300~\text{nK}$) falls below the single-photon recoil limit ($T_r = \hbar^2 k^2 / m k_B = 283~\text{nK}$). Consequently, the ultimate red MOT temperature is bounded by the photon recoil limit.
...
$m_F=9/2 \rightarrow m_F'=7/2$. The relative probability of these two processes is
set by their Clebsch-Gordan coefficients; the transition from $m_F=9/2$ to $m_F=7/2$
is less probable than to $m_F=11/2$, which settles the probable of atoms pushed to the
extreme out positions of the trap.

The dominant loss mechanism is instead geometric. The restoring force for an atom
in $m_F=9/2$ is maximal on one side of the trap only; on the opposite side, this
same atom cannot come into resonance with either beam at all, and is lost
\cite{stellmerThesis}. This is especially detrimental for the stretched states
$m_F=\pm9/2$: because their cycling transition is \textit{closed}, these atoms
have no spontaneous decay path to another $m_F$ state that could rescue them once
they find themselves on the wrong side of the trap -- unlike atoms in intermediate
$m_F$ states, which are naturally redistributed by the trapping light itself as
soon as they scatter a photon elsewhere.
...
A dedicated stirring laser mitigates this loss by driving the open
$^1S_0 (F=9/2) \rightarrow {}^3P_1 (F=9/2)$ transition, with its frequency swept in
a sawtooth pattern \textit{in time}, as shown in Fig.~\ref{fig:intro-randomisation}
(right panel). At any given instant, this fixed frequency is simultaneously
resonant with several $m_F$ sub-levels at different positions across the trap
(left panel); as the frequency sweeps, the ground-state $m_F$ population is
continuously and randomly redistributed. Because the differential Zeeman shift of
$F'=9/2$ is about five times smaller than that of $F'=11/2$
\cite{stellmerThesis}, its resonance region is correspondingly broader in space,
allowing the stirring light to reach and randomize atoms over a much wider
portion of the trap than the narrow-line trapping transition alone. This
continuous optical pumping brings atoms that would otherwise remain trapped in an
unfavorable $m_F$ state on the wrong side of the trap back within reach of the
cycling transition, restoring efficiency to the cooling cycle.
...
{}^3P_1(F=9/2)$ transition]{Illustration of the melange concept on the
$^1S_0(F=9/2) \rightarrow {}^3P_1(F=9/2)$ transition, added to the red MOT
cooling to capture extreme spin-state components inaccessible to the
counter-propagating lasers of the red MOT. Left: spatial dependence of the
resonance condition on each $m_F$ sub-level, as a function of position $x$
($B\propto x$). Right: the stirring laser's frequency, swept in a sawtooth
pattern \textit{over time}. At each instant $t_0$, the instantaneous frequency
(right) is simultaneously resonant with several $(m_F,x)$ pairs (left, black
dots); sweeping this frequency over time thus randomizes the $m_F$ population
across the whole trap.}
...
As seen from these expressions, the AC Stark shift induced by the dipole laser is
state-dependent. As illustrated in Fig.~\ref{fig:intro-odt-loading}, the potential
estimated in \cite{litvinov_these} is deeper for the ground state $^1S_0$
($U_{\text{dip}}(^1S_0) \simeq -1.5~\text{MHz}$) than for the excited state $^3P_1$
($U_{\text{dip}}(^3P_1) \simeq -1.2~\text{MHz}$) at the center of the trap, owing to its
higher polarizability. This results in a differential \emph{blue}-shift of the
$^1S_0 \rightarrow {}^3P_1$ transition at the trap center, relative to the unperturbed
atomic lines (dashed lines).

The thin red curve tracks the local resonance condition: it represents the energy an
atom would reach by absorbing one $689~\text{nm}$ photon from its local $^1S_0$ level,
and its crossing with the shifted $^3P_1$ curve marks where the laser is exactly
resonant. Since the $689~\text{nm}$ MOT laser is chosen slightly blue-detuned from the
unperturbed transition, this differential shift brings it \emph{red}-detuned from the
local resonance at the trap center, while it remains blue-detuned at the edges, where
the light shift vanishes. As a result, atoms near the center scatter photons less
often, but each scattering event cools them; atoms near the edges scatter more often,
but each event slightly heats them, since the local detuning there is blue. Atoms that
retain too much thermal energy from this imbalance are eventually lost from the trap.
...
Following loading, forced evaporative cooling is performed to reach deep Fermi degeneracy, down to temperatures on the order of $T = 100~$nK, $T/T_F=0.25$. Evaporative cooling is achieved by progressively lowering the trap depth: the most energetic atoms overcome the potential barrier and escape the trap, while the remaining ones re-thermalize to lower temperatures via elastic collisions (Fig.~\ref{fig:evap}).
