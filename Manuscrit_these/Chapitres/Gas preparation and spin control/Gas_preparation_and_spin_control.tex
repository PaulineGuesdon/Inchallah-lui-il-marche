...
Due to anisotropic geometric constraints of our atomic beam, the intensity in the horizontal arm is set roughly three times higher than in the vertical arm, \colorbox{blue}{puissance pour $nI_{sat}$} though both remain below saturation ($I \ll I_{\text{sat}}$) over an interaction area of approximately $10~\text{cm}^2$. The TC laser remains active throughout subsequent blue-cooling stages to ensure continuous atomic flux collimation.
...
\begin{figure}[H]
    ...
    \label{}
\end{figure}
\hl{Stir valeur identique pour sûr. Verifier pompopt et inj slave si bien identiques. Si inj slave pareil, alors MOT rouge pareil}
