...
\begin{sidewaysfigure}
    ...
    \textbf{First:} magnetic field gradient (BMOT) and the three bias/compensation coil voltages (VA, VC, VD). \textbf{Second:} intensities of the $461~$nm, $403~$nm, $689~$nm, and $1075~$nm light. \textbf{Third:} AOM frequency of the entire red cooling stages. \textbf{Fourth} Real frequencies
    compared to the 9/2 state for the stirring laser and 11/2 for the MOT. Plain lines is the real experimental rampes and dashed rampes are the ideal ones for the optimization of the experiment (see more details in \ref{BB MOT}). Frequencies of 
...
\subsubsection{Swap cooling, broadband MOT to narrowband}
\label{BB MOT}
...
two successive stages. In parallel a redistribution of the spin is assured by a stirring 
laser, its function explained in the following subsection. 

In the first stage, lasting $182~\text{ms}$, SWAP (Sawtooth Wave Adiabatic Passage) cooling is applied
\cite{bartolotta_laser_2018}. As illustrated in \ref{fig:swap-mechanism} two counter-propagating beams have their frequency swept through resonance, 
then reset almost instantaneously back to the initial frequency
of the ramp. The ramp drives two sequential, stimulated transfers: first
absorption from the beam the atom is moving towards. The laser gets on resonance with the 
Doppler shifted atom at $\omega_L - kv$. The frequency of the laser still increasing then 
becomes resonant with the opposite polarized laser which affects the atom with a new velocity
v' after the absorption: by emission into the opposing beam $2\hbar k$ are removed of momentum 
per sweep without relying on spontaneous emission. The fast reset, in contrast,
is deliberately non-adiabatic so that it drives no transfer, and hence no momentum kick,
back the other way; a symmetric ramp would instead undo the cooling just achieved. 


\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Chapitres/Gas preparation and spin control/images/swap_mechanism.pdf}
    \caption[Mechanism of SWAP (sawtooth-wave adiabatic passage) cooling]{Mechanism of SWAP cooling, adapted from \cite{bartolotta_laser_2018}. Left: over one slow rise of the sawtooth $\omega_L(t)$, the laser successively crosses the two Doppler-shifted resonance conditions $\omega_a - kv$ and $\omega_a + kv'$, driving first an absorption then a stimulated emission into the opposing beam; the fast reset transfers no population. Top right: the corresponding two-level picture, showing the atom transiently promoted to $\ket{e}$ before returning to $\ket{g}$ with a net momentum change of $-2\hbar k$. Bottom right: geometry, with the atom (velocity $v$) between two counter-propagating beams sharing the same swept frequency $\omega_L(t)$.}
    \label{fig:swap-mechanism}
\end{figure}


The trapping beam (MOT) is swept between RF frequencies $111.9$ and $110.4~\text{MHz}$ -- corresponding to a
...
an intensity of $I_{A100}(MOT87) = 5~mW/cm^2$ and $I_{A100}(STIR) = 2~mW/cm^2$.
...
After $9.3~\text{ms}$, the repumper pulse
described above interrupts the SWAP of the MOT and STIR sweeps for $3.2~\text{ms}$; the MOT and STIR sweeps then
...
still $I_{A85} = 2~mW/cm^2$ for the STIR beam by the end of the stage. Toward the end of this
first stage, the sweep transitions into a broadband-capture phase, using a symmetric triangular
ramp (no fast reset) to sweep continuously across a broader range of Zeeman sub-levels, as
illustrated in Fig.~\ref{fig:intro-red-mot-3-steps}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Chapitres/Gas preparation and spin control/images/red_MOT_in_3_steps.pdf}
    \caption[Broadband capture stage of the $689~\text{nm}$ red MOT.]{Broadband capture stage of the $689~\text{nm}$ red MOT (adapted from \cite{stellmerThesis}). Top: Zeeman sublevels depending on the B field and position since $B \propto x$. Dark dots show the Zeeman states adressed
    at $t_0$ by the laser rampe plotted on the bottom figure. The orange dots are the Zeeman states adressed at $t_1$ by the same laser rampe.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Chapitres/Gas preparation and spin control/images/1.30_dipole_trap(1075)_setup.png}
...
frequency or magnetic field calibration \cite{Bataille2020}.
...
\caption[Spin manipulation scheme extracted from \cite{bataille_realisation_nodate}]{Spin manipulation scheme of \cite{bataille_realisation_nodate}. The isolation of spin states is allowed by the tensorial shift of the ground states that makes neighbours $m_F$ states isolated from the other resonances. The Bloch sphere represents the reduced Hilbert space of the selected qubit.}
...
\subsubsection{Experimental setup of the Raman and PA process}
