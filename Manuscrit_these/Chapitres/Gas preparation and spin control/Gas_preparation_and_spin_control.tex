...
For $t_{\text{BlueMot}} = 8.2~\text{s}$, 2D transverse cooling (TC) collimates atoms prior to a Zeeman slowing stage, followed by a blue Magneto-Optical Trap (MOT), operating on the broad $^1S_0 \rightarrow {}^1P_1$ transition (see Fig.~\ref{energy levels sr}). Following this blue stage, repumping light is applied for $t_{\text{Repump}} = \text{\textcolor{blue}{time}}$, swept across multiple hyperfine states of the $^3P_2 \rightarrow {}^3D_2$ transition to recover approximately $25\%$ of the atoms trapped in the metastable $^3P_2$ state. The atoms are then cooled to temperatures close to $T = \text{\hl{temperature}}$ by a dynamic red MOT for $t_{\text{Red}}^{\text{tot}} = \text{\textcolor{blue}{time}}$ on the narrow $^1S_0 \rightarrow {}^3P_1$ transition. Finally, the gas is loaded and evaporated inside an Optical Dipole Trap (ODT).
...
\begin{sidewaysfigure}
    ...
    \caption{Experimental sequence for the magnetic field, lasers intensity and frequencies depending on the time. In the bottom right corner is the detailed imaging sequence. }
...
The transition to the next step necessits the extinction of the blue lasers while keeping the same magnetic field to avoid loosing the atoms that decayed to the $^3P_1$ state to end up in the ground state. Then, 
before turning on the repumper laser, one decays slightly the magnetic field to value \colorbox{blue}{B=} and turning on the red MOT laser to maximize the recovering of the repumped atomes that necessits really short 
time.
...
The repumper frequency scan range is optimized around $3~\text{GHz}$ to selectively cover the $F=11/2$ and $F=13/2$ states. Scanning a wider range reduces total atom recovery by decreasing the dwell time on these critical transitions. At the end of the blue MOT phase, the $461~\text{nm}$ beam is extinguished while the $403~\text{nm}$ repumper is turned on for 3.2~ms at $I \approx 15~\text{mW/cm}^2$.
Meanwhile the red MOT lasers are turned on for a best charging in the red MOT so they are not lost on the decay of $^3D_2$ states and keep in the cycle.
...
A double-pass AOM fine-tunes the frequency before splitting into three spatial counter-propagating beam pairs.
...
gradient, operated in two successive stages.

In the first stage, SWAP (Sawtooth Wave Adiabatic Passage) cooling is applied
...
back the other way; a symmetric ramp would instead undo the cooling just achieved. As the
cloud cools and compresses during this stage, both the modulation amplitude $\Delta f$ and
the total optical power are ramped down. This process is illustrated by the left scheme of 
\ref{fig:intro-red-mot-3-steps} that represents the projection total spin of the electrons
with the frequency ramp red detuned by -18~MHz.

In the second stage visible at the right of \ref{fig:intro-red-mot-3-steps}, the cooling light is modulated with a conventional sawtooth ramp
detuned from resonance, operating as a standard frequency-modulated Doppler-cooling MOT
that further compresses and cools the now colder, denser cloud.
...
\begin{figure}[H]
    ...
    \caption{Counterpropagating lasers in resonance with lightshifted $m_f$ states of $^3P_1, F=11/2$ hyperfine state.}
...
\begin{figure}
    ...
    \caption{Illustration of the melange concept added to the red MOT cooling to capture extreme spin states components unaccessible from the counterpropagating lasers of the red MOT.}
...
In practice, the broadband/SWAP red MOT operates with a total intensity well above
saturation (typically $s \sim 10^{2}\text{--}10^{3}$), since the frequency modulation
distributes this power over a broad spectral range. Because the laser is only resonant
with a given velocity class for a small fraction of the sweep period, the time-averaged
saturation -- and hence the effective power broadening -- experienced by any individual
atom remains low, despite the large total intensity. To support the atomic cloud against
gravity, a higher intensity is delivered along the vertical axis than along the horizontal
axes, ensuring the radiation pressure force exceeds $mg$ in the direction where it is
needed most, while limiting unnecessary power broadening and transverse heating on the
horizontal axes.

During all the process of swap and sweep frequency the intensity and time duration of each stage is simply experimentally optimized, always with a purpose of decreasing the intensity when going to the end of the cooling
to reduce any heating and spread of the gas.
...
This differential shift creates a localized spatial resonance condition: atoms at the center of the trap remain in resonance with the red MOT beams, where they are continuously cooled and trapped. In other words a slight red detuning in the center of the trap corresponds to a blue detuning outside of the center. Conversely, energetic atoms at the periphery experience a negligible light shift and remain blue-detuned from the local resonance, preventing unwanted re-heating.
...
\begin{figure}[H]
    ...
    \caption{Evaporative cooling mechanism in a crossed optical dipole trap, illustrating the escape of high-energy atoms and subsequent re-thermalization.}
    \label{fig:evap}
\end{figure}
...
\hl{Magnetic field bias and quantization axis: To preserve spin polarization, the magnetic field is never allowed to cross zero during transitions between cooling phases. Maintaining non-zero bias components along at least two orthogonal directions (e.g., $|B_x|$ and $|B_y|$) preserves the quantization axis and prevents spin depolarization.}
...
Once locked to the injection laser, the optimized cavity achieves a total power transmission efficiency of $T = 0.63$. This efficiency could come from a mis-gluing of the mirror that gives an angle to the optical direction of the cavity, and a part of the beam could clip inside of it. It could also come from dirt, maybe the glue. 
...
\begin{figure}[H]
    ...
    \caption{Homemade PID scheme composed by the separated proportionnal and integrated part we considered sufficiant for our setup.}
    \label{lock PID}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Closed-loop noise analysis and mechanical resonance identification using the Red Pitaya with a gain that is amplifying parasit noise, such as the mechanical resonance of the piezo at $f = 35~$kHz.}
