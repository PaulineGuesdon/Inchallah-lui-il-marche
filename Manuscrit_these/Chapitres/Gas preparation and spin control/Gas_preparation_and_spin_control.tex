...
\begin{equation}
    N(t) = N_{sat}(1-e^{-t/\tau})
\end{equation}
$N_{sat} = \alpha \Gamma \tau$ when the transition reaches its maximum value. To saturate the blue MOT atom number one should wait for few blue MOT lifetime which corresponds to 100~ms to 1~sec. \\
...
The repumper frequency scan range is optimized around $3~\text{GHz}$ to selectively cover the $F=11/2$ and $F=13/2$ states. Scanning a wider range reduces total atom recovery by decreasing the dwell time on these critical transitions. At the end of the blue MOT phase, 
the $461~\text{nm}$ beam is extinguished to avoid them to recycle on the blue transition after the repump. The $403~\text{nm}$ repumper is turned on for $3.2~\text{ms}$ at $I \approx 15~\text{mW/cm}^2$. This pulse begins $9.3~\text{ms}$ after the $689~\text{nm}$ red MOT lasers to avoid losing atoms decaying from $^3D_2$ states, and to keep them within the red cooling cycle.
