...
\begin{sidewaysfigure}
    ...
    are the mean intensity in each fiber of the cooling stage (e.g intensity in 1/3 of the MOT fibers). Both real physicals values and instructions on the variable attenuator (VA,VC,VD, BMOT) and the frequencies on the AOM's  are writen for the reader and the experimentators of this machine. 
    \textbf{Top:} magnetic field gradient (BMOT) and the three bias/compensation coil voltages (VA, VC, VD). \textbf{Middle:} intensities of the $461~$nm, $403~$nm, $689~$nm, and $1075~$nm light. \textbf{Bottom:} frequency detunings of the entire red cooling stages, and of the reservoir/dimple beams during evaporation. 
...
\end{equation}
where $\Phi$ is the raw atomic flux incident on the MOT capture region, $\alpha$ is the dimensionless capture efficiency (the fraction of that flux actually trapped, depending on experimental parameters), $\gamma_{loss} = \gamma_{vacuum} + \gamma_{leak}$ respectively the loss rate due to the imperfection of the vacuum and the rate leakage to the
metastable state. Then the number of atom in time evolves as:
\begin{equation}
    N(t) = N_{sat}(1-e^{-t/\tau})
\end{equation}
$N_{sat} = \alpha \Phi \tau$ when the transition reaches its maximum value. Since the blue MOT lifetime is dominated by the leak into the metastable state and amounts to a few tens of milliseconds \cite{stellmer_degenerate_2013}, saturating the blue MOT atom number only requires waiting for a few such lifetimes. \\
...
$^1S_0, F=9/2$ and excited $^3P_1, F'=11/2$ hyperfine levels. For a $\sigma^+/\sigma^-$
MOT to provide a restoring force at every position, the ratio of excited to
ground-state Landé factors must satisfy $\frac{F}{F+1} < \frac{g_e}{g_g} <
\frac{F}{F-1}$ \cite{stellmerThesis}. In alkali atoms this is comfortably satisfied,
since the ground-state $g_g$ is of the same order as $g_e$. Here however, the
$^1S_0$ ground state has $g_g\approx0$ (only a tiny nuclear-magnetic-moment
contribution survives, no electronic contribution since $J=0$), so that
$g_e/g_g\gg1$: the double inequality is badly violated, and the restoring force
is only local rather than guaranteed everywhere in the trap. As illustrated in
...
soon as they scatter a photon elsewhere. Note that the trapping beam itself is also
frequency-broadened to enlarge its capture range (see the broadband MOT stage above),
but this does not help: it still only drives the closed transition, so no amount of
frequency coverage opens a spontaneous-decay path out of the stretched $m_F$ states.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{Chapitres/Gas preparation and spin control/images/need_for_stir.pdf}
...
drifting. When the stable mode of the laser is not accessible anymore with the possible intensity diode one might still low down the threshold of the laser. The final way 
