...
Because the transition is not closed, atoms accumulate in $^3P_2$. $t_{\text{BlueMot}} = 8~\text{s}$ of accumulating 
more and more atomes in this state (trapped magnetically): the sequence switched to the next stage of manipulation. 
The atoms are repumped in 3.2 ms by a frequency sweep across multiple hyperfine states of the
$^3P_2 \rightarrow {}^3D_2$ transition, to recover approximately $25\%$ of the atoms, and immediatelty recaptured in 
a MOT on the narrow line.
