...
To connect this scattering amplitude to observable quantities, one must translate these individual collision events into a population decay. Since the photoassociation process requires two atoms to collide, the loss rate scales quadratically with the atomic density. Integrating this local behavior over the trap volume yields the rate equation for the total atom number:
\begin{equation}
    \frac{dN}{dt} = - \beta N^2 - \Gamma N
    \label{equation one and two-body losses}
\end{equation}
Here, the linear term $\Gamma N$ accounts for independent one-body collisions with the residual background gas, while the quadratic term driven by $\beta$ directly reflects the light-induced two-body losses quantified by $a''$.
...
Different processes can occur for the molecule to leave the trap: by spontaneous emission
it can come back to a scattering channel and gain enough kinetic
energy to leave the trap. Spontaneous emission can also lead to a deeply bound state in
the ground state potential. \hl{ajouter ref ?}\\
...
\begin{equation}
    \delta a = \frac{l_{opt}\Gamma_{mol}\Delta}{\Delta ^2 + \frac{(\eta\Gamma_{mol})^2}{4}}
\end{equation}
Here, $l_{opt}$ is the \textbf{optical length}, a central parameter characterizing the strength of the coupling between the scattering atoms and the bound molecular state. It scales linearly with the laser intensity $I$ according to \cite{reschovsky_narrow-line_2018}:

\begin{equation}
    l_{opt} = \frac{\lambda^3}{16\pi c}\frac{f_{FC}}{k}f_{rot}I = A I
    \label{eq:lopt_definition}
\end{equation}

where $\lambda = 689.45$~nm is the transition wavelength, $f_{rot}$ is a rotational factor ($f_{rot} = 1$ for $0_u^+$ resonances and $f_{rot} = 2$ for $1_u$ resonances), and $A$ is an experimental constant. Crucially, the coupling amplitude depends on the spatial overlap between the initial ground state $\psi_g(r)$ and final excited state $\psi_e(r)$ wavefunctions through the Franck-Condon factor:
\begin{equation}
...
\begin{equation}
    \delta a \propto \frac{I\gamma_{mol}\Delta}{\Delta^2 + (\eta \gamma_{mol})^2/4}
\end{equation}


\subsection{General scaling laws for PA}
We will discuss in this subsection the dependence of the PA rate $K_2$ on different parameters. The latter can be expressed as \cite{blatt_systematic_2011}:
\begin{equation}
    K_2 = \frac{8\pi \hbar}{m_{at}}\times \frac{l_{opt}\Gamma_{mol} \gamma}{\Delta^2 +\frac{1}{4}(1 + 2k l_{opt}\Gamma_{mol})^2} % Attention j'ai rajouté \Gamma_mol qui manquait souvent dans cette forme
    \label{K2 complet}
\end{equation}
where $\Delta$ is the detuning from the molecular resonance and $m_{at}$ is the atomic mass.
The relation between the two-body losses rate $K_2$ and the fitting parameter $\beta$ demonstrates as:
\begin{gather}
...
For low temperatures, the de Broglie wavelength settles the negligibility of the wavevector $k$ ($k \to 0$), which simplifies Eq.~\ref{K2 complet}. On resonance ($\Delta = 0$), the PA rate reduces to:
\begin{equation}
    K_2 = \frac{16h}{m_{at}}l_{opt}
\end{equation}
As established in Eq.~\ref{eq:lopt_definition}, this shows that at low temperatures, the maximum photoassociation rate becomes purely linear with the laser intensity $I$.
