...
In the atomic pair frame, the electric dipole interaction drives the system from the ground electronic state $\psi_g$ of potential $V_g(r)$ to the excited molecular state $\psi_e$ of potential $V_e(r)$. In the absence of a resonant laser, the internal evolution of the system is purely elastic and expresses as:
\begin{equation}
    i\hbar \frac{d}{dt}
    \begin{pmatrix}
        \psi_g\\
...
When a laser is tuned near a specific vibrational level of the excited potential $V_e(r)$, it couples the free-atom continuum to the bound molecular state, as illustrated in Fig.~\ref{fig:PA_concept}.
\begin{figure}
    \centering
...
\begin{equation}
    ...
    \begin{pmatrix}
        ...
        \hbar\Omega/2 & -\frac{\hbar^2}{2\mu}\nabla^2 + V_e + \hbar \Delta -i\hbar\frac{\Gamma_{\text{mol}}}{2}
    \end{pmatrix}
    \begin{pmatrix}
        \psi_g\\
        \psi_e
...
This matrix equation shows that the laser creates a bridge between the two manifolds. During a collision, two atoms approaching each other on the ground potential $V_g(r)$ have a non-zero probability to absorb a photon and transition to the excited molecular potential $V_e(r)$. Once populated, this state decays via spontaneous emission at a rate $\Gamma_{\text{mol}}$. To account for this particle loss within the quantum mechanical framework, the molecular decay appears as an imaginary term $-i\hbar\frac{\Gamma_{\text{mol}}}{2}$ in the Hamiltonian matrix. In quantum scattering theory, such an imaginary component acts as a sink that absorbs probability flux, directly translating the physical disappearance of the atoms from the elastic channel.

Since the laser is tuned far from any bare atomic resonance, the excited molecular state remains weakly populated, meaning the atoms spend most of their time interacting on the ground-state potential. We can therefore project the effect of the excited state directly onto the ground state. Consequently, at large distances, the stationary solution takes the form of a spinor $\ket{\psi_g} + \epsilon \ket{\psi_e}$ with $\epsilon \ll 1$. The asymptotic wavefunctions carry the signatures of the loss events that occurred at short distance, forcing the scattering phase shift $\delta_l$ to become a complex number. 

The second line of the coupled equations yields the expression for the excited bound-state wavefunction $\psi_e$:
\begin{gather}
    \frac{\hbar \Omega}{2}\psi_g + \left(-\frac{\hbar^2\nabla^2}{2\mu} + V_e - \hbar \Delta - i\hbar\frac{\Gamma_{\text{mol}}}{2}\right)\psi_e = E\psi_e \\[1.5ex]
    \psi_e = \frac{\frac{\hbar\Omega}{2}}{E + \frac{\hbar^2 \nabla^2}{2\mu} - V_e + \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}}\psi_g
\end{gather}
When injected back into the first coupled equation, one obtains the effective single-channel Schrödinger equation for the ground state:
\begin{gather}
    \left(-\frac{\hbar^2\nabla^2}{2\mu} + V_g \right)\psi_g = E\psi_g - \frac{\hbar\Omega}{2}\psi_e\\[1.5ex]
    \left(-\frac{\hbar^2\nabla^2}{2\mu} + V_g \right)\psi_g = \left( E - \frac{\left(\frac{\hbar\Omega}{2}\right)^2}{E + \frac{\hbar^2\nabla^2}{2\mu} - V_e + \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}} \right)\psi_g
    \label{psi e dans psi g}
\end{gather}
Because in scattering theory $\delta a$ is induced by projecting what we call the optical potential - one identify by the second member of right part of \ref{psi e dans psi g} - in the ground wavefunction $\psi_g$.
Then at resonance $E + \frac{\hbar^2\nabla^2}{2\mu} - V_e = 0$ and $a(\Delta)$ is:
\begin{equation}
    a(\Delta) = a_{bg} + \frac{l_{opt}\Gamma_{mol}}{\Delta + i\frac{\Gamma_{mol}}{2}}
    \label{a(delta)}
\end{equation}
also demonstrated in \cite{bohn1997_prospects}, where $a_{\text{bg}}$ is the real background scattering length representing native elastic collisions, and $\Gamma_{\text{mol}}$ is the natural molecular decay width. The light-induced stimulated linewidth $\Gamma_{\text{stim}}(k)$ accounts for the coherent laser coupling and scales linearly with the collision wavevector $k$ at low temperature according to \cite{tiesinga2005measurement}:
\begin{equation}
    \Gamma_{\text{mol}} = \frac{\Gamma_{\text{stim}}(k)} {2 k l_{\text{opt}}}
    \label{eq:gamma_stim_definition}
\end{equation}
Here, $l_{\text{opt}}$ is the temperature-independent optical scattering length, which acts as the central parameter characterizing the strength of the coupling and scales strictly linearly with the laser intensity $I$.

By multiplying Eq.~\ref{a(delta)} by the complex conjugate of its denominator, we can decouple the scattering length into its real and imaginary parts
\begin{gather}
    a(\Delta) = a_{\text{bg}} + \frac{l_{\text{opt}}\Gamma_{\text{mol}}\left(\Delta - i\frac{\eta \Gamma_{\text{mol}}}{2}\right)}{\Delta^2 + \left(\frac{\eta \Gamma_{\text{mol}}}{2}\right)^2}\\[1.5ex]
    a(\Delta) = a_{\text{bg}} + \frac{l_{\text{opt}}\Gamma_{\text{mol}}\Delta}{\Delta^2 + \left(\frac{\eta \Gamma_{\text{mol}}}{2}\right)^2} - i \frac{\frac{1}{2} l_{\text{opt}}\Gamma_{\text{mol}}^2 \eta}{\Delta^2 + \left(\frac{\eta \Gamma_{\text{mol}}}{2}\right)^2}
\end{gather}

\subsection{General scaling laws for PA}
To connect this microscopic scattering amplitude to observable experimental quantities, one must translate these individual collision events into a population decay. In quantum scattering theory, the fundamental two-body loss rate coefficient $K_2(k)$ is directly driven by the absorptive part of the scattering length through the relation $K_2(k) = \frac{8\pi\hbar}{m_{\text{at}}} a''(k, \Delta)$. Grounded on the systematic study of optical Feshbach resonances by Blatt et al. \cite{blatt_systematic_2011}, substituting Eq.~\ref{eq:a_prime_prime_OFR_final} into this relation yields the complete expression for the loss coefficient as a function of the collision wavevector $k$:
\begin{equation}
