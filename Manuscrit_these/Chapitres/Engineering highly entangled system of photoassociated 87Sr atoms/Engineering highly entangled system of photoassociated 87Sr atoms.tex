...
Following the foundational framework of Breit-Wigner parameters adapted to laser-assisted collisions \cite{bohn1997_prospects}, this complex phase shift allows one to naturally define a unified complex effective scattering length $a(k, \Delta)$ that explicitly accounts for the light-induced coupling width:
\begin{equation}
    a(k, \Delta) = a_{\text{bg}} + \frac{1}{k} \frac{\frac{1}{2}\Gamma_{\text{stim}}(k)}{\Delta + i \frac{\Gamma_{\text{mol}} + \Gamma_{\text{stim}}(k)}{2}}
    \label{eq:complex_a_with_Gammastim}
\end{equation}
where $a_{\text{bg}}$ is the real background scattering length representing native elastic collisions, and $\Gamma_{\text{mol}}$ is the natural molecular decay width. The light-induced stimulated linewidth $\Gamma_{\text{stim}}(k)$ accounts for the coherent laser coupling and scales linearly with the collision wavevector $k$ at low temperature according to \cite{tiesinga2005measurement}:
\begin{equation}
...
By multiplying Eq.~\ref{eq:complex_a_with_Gammastim} by the complex conjugate of its denominator, we can decouple the scattering length into its real and imaginary parts, $a(k, \Delta) = a_{\text{bg}} + \delta a(k, \Delta) + i a''(k, \Delta)$. In the limit where the stimulated width remains small compared to the natural decay ($\Gamma_{\text{stim}} \ll \Gamma_{\text{mol}}$ at the numetator), this algebraic separation yields the standard dispersive and absorptive profiles used in modern Optical Feshbach Resonance (OFR) descriptions \cite{yan_controlling_2013}:
\begin{gather}
    \delta a(k, \Delta) = \frac{l_{\text{opt}}\Gamma_{\text{mol}}\Delta}{\Delta ^2 + \frac{1}{4}\big(\Gamma_{\text{mol}} + \Gamma_{\text{stim}}(k)\big)^2} \label{eq:delta_a_OFR_final} \\
    a''(k, \Delta) = \frac{l_{\text{opt}}\Gamma_{\text{mol}}\frac{\Gamma_{\text{mol}}}{2}}{\Delta ^2 + \frac{1}{4}\big(\Gamma_{\text{mol}} + \Gamma_{\text{stim}}(k)\big)^2} \label{eq:a_prime_prime_OFR_final}
\end{gather}
The real part $\delta a$ represents the light-induced shift of the elastic properties, whereas the imaginary part $a''$ directly quantifies the rate of inelastic two-body losses.
