...
\begin{gather}
E \Psi_e = \frac{\hbar \hat{\Omega}}{2}\Psi_g \left(\hat{H_e} + \hbar \Delta - i\hbar \frac{\Gamma_{mol}}{2} \right)\Psi_e\\
\bra{v_e}(E-\hat{H_e}\Psi_e) = \bra{v_e}\frac{\hbar\hat{\Omega}}{2}\bra{\psi_g} \\
\psi_e(r) \approx \frac{\left\langle v_e \middle| \frac{\hbar\Omega(r)}{2} \middle| \psi_g \right\rangle}{E - E_{v_e} + \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}} v_e(r)
\end{gather}

Substituting this spatial projection back into the ground-state channel (first line of Eq.~\ref{inelastic evolution equation}) yields an effective single-channel Schrödinger equation:
\begin{gather}
    i\hbar\frac{d}{dt}\Psi_g = \left(-\frac{\hbar^2\hat{\nabla^2}}{2\mu}-\hat{U_g}\right)\Psi_g +\frac{\hbar\Omega}{2}\Psi_e\\
    \left(E+\frac{\hbar^2\hat{\nabla^2}}{2\mu} - \hat{U_g}\right) \Psi_g = \frac{\hbar\hat{\Omega}}{2} \Psi_e\\
    \left(E+\frac{\hbar^2\hat{\nabla^2}}{2\mu} - \hat{U_g}\right) \Psi_g = \frac{\hbar\hat{\Omega}}{2} \frac{\frac{\hbar\hat{\Omega}}{2}\ket{v_e}\bra{v_e}\hbar\hat{\Omega}}{2}}{E-E_{v_e} + \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}}\ket{\Psi_g}\\
\end{gather}

\begin{equation}
\left(-\frac{\hbar^2\nabla^2}{2\mu} + U_g(r) + \hat{U}_{opt}\right)\psi_g(r) = E \psi_g(r)
\label{psi e dans psi g}
\end{equation}
where the non-local complex optical potential $\hat{U}_{opt}$ evaluates on the ground state as:
\begin{equation}
\langle \psi_g | \hat{U}_{opt} | \psi_g \rangle = \frac{\left| \left\langle v_e \middle| \frac{\hbar\Omega(r)}{2} \middle| \psi_g \right\rangle \right|^2}{E - E_{v_e} + \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}}
\end{equation}

Following Bohn and Julienne \cite{bohn1997_prospects}, integrating this optical potential within the asymptotic boundary conditions for $r \to \infty$ modifies the scattering phase shift, leading to the complex scattering length $a(\Delta)$:
\begin{equation}
