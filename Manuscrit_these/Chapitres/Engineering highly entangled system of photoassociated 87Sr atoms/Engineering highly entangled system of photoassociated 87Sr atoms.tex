...
\begin{equation}
    a(\Delta) = a_{\text{bg}} + \frac{l_{\text{opt}}\Gamma_{\text{mol}}\Delta}{\Delta^2 + \left(\frac{\Gamma_{\text{mol}}}{2}\right)^2} - i \frac{l_{\text{opt}}\Gamma_{\text{mol}}\frac{\Gamma_{\text{mol}}}{2}}{\Delta^2 + \left(\frac{ \Gamma_{\text{mol}}}{2}\right)^2}
\end{equation}
...
To connect this microscopic scattering amplitude to observable experimental quantities, one must translate these individual collision events into a population decay that writes as \cite{martinez_de_escobar_two-photon_2008}:
\begin{equation}
...
\end{equation}
the first and second terms representing the kinetic energy and radial energy of the molecule. Higher-order electronic interactions have been neglected depending on the relevant Hund's case. For $^{87}\text{Sr}$, the Born-Oppenheimer potential $\hat{U}_{\text{BO}}$ is approximated using a long-range expansion similar to $^{173}\text{Yb}$:
\begin{equation}
    U_{\text{BO}}(r) = -\frac{C_6}{r^6} \left( 1 - \frac{\sigma^6}{r^6} \right) - s \frac{C_3^{\Omega}}{r^3}
    \label{eq:sr_potential}
\end{equation}
where $s = +1$ and $s = -1$ correspond to $g$ and $u$ potentials, respectively. $C_6$ and $C_3^{\Omega}$ are Van der Waals and dipole-dipole coefficients \cite{borkowski_line_2009}, $\Omega$ still standing for projection of angular momentum, and $\sigma$ is a free parameter to adjust with the experimental results.
...
\end{figure} 
In conclusion, combining semi-classical WKB theory with LeRoy-Bernstein scaling offers a practical framework to estimate vibrational spacings near the intercombination threshold. This approach serves as a reliable guide for bosonic isotopes like $^{88}\text{Sr}$, presented in the next section. However, it cannot account for the $^{87}\text{Sr}$ photoassociation spectra, where instead of the dense manifold of lines, only a few narrow resonances are experimentally observed.
