...
\begin{equation}
    \ket{\Psi} = \left(\ket{\beta_1}_A \ket{\beta_2}_B -\rho \times p \ket{\beta_1}_B \ket{\beta_2}_A\right)
     \otimes\ket{\phi}
     \label{wavefunction parity}
\end{equation}

the coefficients $p_i= (-1)^R$ the parity of the electronic state i and $\rho = p_1 p_2$ the product of the parity of ground and excited
states. In the case of the fermions $\rho = -1$ and $\rho = +1$ for bosons, which constrains the \textbf{rotational quantum number $R$} to values $R = 1, 3$ or $5$... 
\cite{reichenbach_optical_nodate}.\\

The rotational quantum number $R$ describes the relative orbital motion of the two colliding nuclei. Under the exchange of the two identical atoms, the spatial rotational wavefunction acquires a parity phase of $(-1)^R$. Due to the overall exchange symmetry dictated by Pauli's principle, a given electronic and spin state only allows specific values of $R$ (e.g., odd $R$ for $p$-wave collisions or even $R$ for $s$-wave collisions).
For each value of R the electronic spatial wavefunction $\ket{\Psi_{\pm}}$ can be odd or even under nuclei exchange. This symmetry is denoted by the terms \textbf{gerade} (even) and \textbf{ungerade} (odd). Figure \ref{fig:u-g symmetry}
is a representation of the probability density of the electronic wavefunctions : we see that for the odd wavefunction cancels the probability of finding
...
The spin-orbit coupling also plays a crucial role in determining the molecular structure. In this context, the total angular
...
to the internuclear axis. This regime is quantified by $\Omega$ and is appropriately described within the framework of Hund's case (c) (see Fig.\ref{fig:hund}).\\
...
Applied to the case of $^{87}Sr$, we will photoassociate atoms in the $F=11/2$ hyperfine state. \\
Say we have one atom in the $^1S_0$ state with maximum value $f_1=9/2$ and one in the $^3P_1$ state at maximum $f_2=11/2$: we can have
an electronic spin $J = 1$ for the two atoms and possible values of the associated total atomic angular momentum F:
\begin{align}
...
with only even values of R in respect with wavefunction parity of \ref{fig:u-g symmetry}.
