...
the incident electric field of frequency $f_0$ fixed at $f_M=40.48$ MHz (see \cite{black_introduction_2001}). The phase modulation gives rise to interferences between the carrier at central frequency 
...
\end{equation}
of FWHM $\Gamma$, centered in $x_0$ with amplitude $\frac{A}{(\Gamma/2)^2}$ and $y_0$ an offset.
...
These resonances present an effective linewidth coming from broadening sources : on the atomic lines the intensity of the laser induces a \textbf{power broadening} of the resonance:
\begin{equation}
    \Gamma_{power} = \Gamma_{1S_0-3P_1}\sqrt{1+\frac{I}{I_{sat}^{eff}}}
\end{equation}

Due to small Franck-Condon factors, power broadening 
of the linewidth remains negligible since the $I_{sat}$ for molecules is way bigger than for atoms \cite{zelevinsky_narrow_2006}.\\
...
The first molecular state shown in the middle panel of Fig.~\ref{88Sr resonance} was detected using an exposure time of $t = 1000$~ms and an intensity of $I = 826$~mW/cm$^2$. This state is located at a detuning of $f = -23.99 \pm 0.01$~MHz from the atomic transition, as measured with the MOGlabs laser system, revealing an \hl{effective linewidth of $\sigma = 192(9)\text{ kHz}$. This includes a power broadening contribution of $74\text{ kHz}$ (due to the $100 I_{\text{sat}}$ illumination) and a thermal broadening estimated at $19\text{ kHz}$.}
...
Furthermore, the measured linewidth for the $f = -222.161(35)$~MHz state remains comparable to what they plotted in their work while we expect a linewidth of $\Gamma = 15$~kHz in average. This extremely larger $\Gamma_{fit}$ comes from technical issues such as the laser sidebands (see section 3.2.2.2) or simple large width of the laser.
To verify if the broadening was technical we measured the same molecular line with another laser source to compare its linewidth.\\
...
Frequency [MHz] (measured) & $ \pm 0.01$ kHz & $23.99 \pm 0.01$ kHz \\ \hline
Intensity ($I$) & $$ mW/cm$^2$ & $$ mW/cm$^2$ \\ \hline
$\sigma_{total}$ (measured) & $115.5 \pm 8.8$ kHz & $344.5 \pm 27.3$ kHz \\ \hline
$\gamma_{thermal}$ (calc.) & $$ kHz & $$ kHz \\ \hline
\textbf{Effective linewidth} & $ \pm 29.24$ kHz & $ \pm 13.6$ kHz \\ \hline
\end{tabular}
\end{table}
...
The MOGlabs source presents a \hl{two times} bigger width than the "11/2" beam. Still the resonances are obtained with this larger source since only this one gives access to a large tunable frequency range.
...
From this equation, one expects two distinct slopes on the decay curve: an initial fast two-body decay regime followed by a slow one-body regime as the atomic density drops. 
