...
When a laser is tuned near a specific vibrational level of the excited potential $U_e(r)$, it couples the free-atom continuum to the bound molecular state, as illustrated in Fig.~\ref{fig:PA_concept}.
\begin{figure}
    \centering
...
\begin{equation}
    \psi_e = \frac{\frac{\hbar\Omega}{2}}{E - \left(-\frac{\hbar^2 \nabla^2}{2\mu} + U_e\right) - \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}}\psi_g
\end{equation}
...
\begin{equation}
    \left(-\frac{\hbar^2\nabla^2}{2\mu} + U_g \right)\psi_g = \left( E - \frac{\left(\frac{\hbar\Omega}{2}\right)^2}{E - \left(-\frac{\hbar^2 \nabla^2}{2\mu} + U_e\right) - \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}} \right)\psi_g
    \label{psi e dans psi g}
\end{equation}
The secondary term on the right-hand side acts as a complex, energy-dependent optical potential $U_{\text{opt}}$. In quantum scattering theory, the modsification of the scattering length $\Delta a$ is directly driven by the projection of this optical potential onto the ground state wavefunction $\psi_g$. Near the molecular resonance, where the relative kinetic energy satisfies $E - \left(-\frac{\hbar^2\nabla^2}{2\mu} + U_e\right) \approx 0$, the integration of this localized coupling yields the unities-normalized complex scattering length originally formulated by Bohn and Julienne \cite{bohn1997_prospects}:
\begin{equation}
...
So far we have described the PA process as if there was only one excited-state molecular potential $U_e$
from the perspective of atom losses.
But the molecules formed in this process have a rich internal structure. Understanding this
structure is therefore essential to optimise $\beta(T)$ experimentally.\\
...
we now want to determine and enumerate the molecular potentials $U_e$ by introducing the molecular quantum
numbers that label these potentials to finally determine their possible transition excited states.\\

In analogy to the atomic case, the Born-Oppenheimer approximation allows for the separation of electronic and 
nuclear motions. Consequently, each specific molecular electronic configuration—defined by its relevant molecular
quantum numbers—gives rise to a distinct adiabatic potential energy curve $U_e(r)$. The primary objective of this 
section is to identify and characterize these molecular potentials, upon which the bound-state wavefunctions 
$\psi_e(r)$ and the overall photoassociation dynamics crucially depend. This framework provides the essential 
foundation for mapping and enumerating the excited vibrational states probed during our spectroscopic measurements.\\

In ultracold gases, only collisions in s-wave are impacted by the interaction potential,
because partial waves of higher $l$ mostly are reflected by the centrifugal barrier before crossing regions where interactions 
are not negligeable. Then in respect with Fermi statistics (related to the Pauli principle) the orbital wavefunction $\ket{\phi}$ 
being symmetric the spin wavefunction should be odd to conserve the antisymmetric nature of the total wavefunction $\ket{\psi}$
for fermions. For two atoms A and B possibly in spin state $\ket{\beta_1}$ or $\ket{\beta_2}$ the expression of total wavefunction
\begin{equation}
...
constrains the \textbf{rotational quantum number $R$} to values $R = 1, 3$ or $5$... \cite{reichenbach_optical_nodate}
...
\begin{align}
\text{For } R=1, &\quad {T = 1, 2, 3... 11}\\
\text{For } R=3, &\quad {T = 1,2... 12}\\
\text{For } R=5, &\quad {T = 1,2 ... 14}
\end{align}
...
can be expressed as:
\begin{equation}
    \hat{H} = \frac{\hat{p_r}^2}{2\mu} + \frac{\hbar^2}{2\mu r^2}R\left(R+1\right) +\hat{U}_{BO} + \hat{H}_{HF}
...
\begin{equation}
    \hat{U}_{BO} = \frac{-C_6}{r^6}\left(1-\frac{\sigma^6}{r^6}\right) - s\frac{C_3^{\Omega}}{r^3}
    \label{ytt potential}
\end{equation}
...
Because $N(t,T)$ depends on the temperature, one wants to re-express Eq.~\ref{eq:final_fit} by explicitly showing the $T$-dependence of $\beta(T)$. The atoms are trapped in an ODT with a mean trapping frequency $\bar{\omega} = (\omega_x \omega_y \omega_z)^{1/3}$. According to the equipartition theorem, the spatial width $\sigma_{i}$ (with $i=\{x,y,z\}$) of the Gaussian 
distribution relates to the temperature $T$ via $\frac{1}{2}m_{\text{at}}\omega_{i}^2\sigma_{i}^2 = \frac{1}{2}k_B T$. This yields the effective volume:

\begin{equation}
    U_e(T) = (2\pi)^{3/2}\left(\frac{k_B T}{m_{\text{at}}}\right)^{3/2} \frac{1}{\omega_x\omega_y\omega_z} = (2\pi)^{3/2}\left(\frac{k_B T}{m_{\text{at}}\bar{\omega}^2}\right)^{3/2}
\end{equation}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{1\linewidth}
        ...
        \caption{\hl{attention premier trait 360 au lieu de 340}Resonance spectrum at $375.74 \pm 0.48$ MHz ($I = 5.2$ W/cm$^2$).}
