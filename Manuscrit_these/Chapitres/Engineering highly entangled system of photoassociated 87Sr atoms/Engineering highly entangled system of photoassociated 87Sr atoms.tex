...
simplification. Due to the large mass ratio $\mu = m_n/m_e \simeq 1000$ -$m_n$ the nucleus mass and $m_e$ the electron mass-
...
\begin{equation}
    \psi(r, \theta,\phi) = U_l(r) Y^m_l(\theta, \phi)
\end{equation}
\\
where the spherical harmonics $Y^m_l(\theta, \phi)$ encode the angular dependance.
$U_l(r)$ is the function describing the radial movement of the bound atoms well described in the frame centered
...
\begin{equation}
    -\frac{\hbar^2}{2\mu_N}\left(\frac{1}{r^2} \frac{d^2 U_l(r)}{dr^2}r^2 - \frac{l(l+1)}{r^2}
    + \hat{V}(r)\right)U_l(r) = E U_l(r)
    \label{radial equation}
\end{equation}
$\mu_N = \frac{m_N^2}{2m_N} = \frac{1}{2m_N}$ and $V(r)$ is the electronic potential.
...
\begin{equation}
    U_l(r) \xrightarrow{r\to\infty} \frac{a_l}{r}sin(kr-\frac{l\pi}{2} + \delta_l)
...
shifts which at low momenta $k$ finds:
 ...
 \end{equation}
\hl{regarder les hypothèses dans le Landau }
...
\end{equation}
\hl{donner lien entre f(theta)et delta}

where $f(\theta)$ is the scattering amplitude which determines whether there are 
inelastic collisions and $\theta$ is the angle between the incident and scattered wavevectors.

\begin{figure}
   \centering    
   \includegraphics[width=0.9\linewidth]{diffusion_sinus.pdf}
