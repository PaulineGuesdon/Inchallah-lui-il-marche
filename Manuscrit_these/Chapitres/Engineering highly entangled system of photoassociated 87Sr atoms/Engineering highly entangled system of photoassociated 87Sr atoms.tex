...
Since the laser is tuned far from any bare atomic resonance, the excited molecular state remains weakly populated, meaning the atoms spend most of their time interacting on the ground-state potential. We can therefore project the effect of the excited state directly onto the ground state. Mathematically, isolating $\psi_e$ from the second line of Eq.~\ref{inelastic evolution equation} yields:
\begin{equation}
    \psi_e = \frac{\frac{\hbar\Omega}{2}}{E - \left(-\frac{\hbar^2 \nabla^2}{2\mu} + V_e\right) - \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}}\psi_g
\end{equation}
Substituting this expression back into the first line of the coupled-channel system leads to an effective single-channel Schrödinger equation for the ground state:
\begin{equation}
    \left(-\frac{\hbar^2\nabla^2}{2\mu} + V_g \right)\psi_g = \left( E - \frac{\left(\frac{\hbar\Omega}{2}\right)^2}{E - \left(-\frac{\hbar^2 \nabla^2}{2\mu} + V_e\right) - \hbar\Delta + i\hbar\frac{\Gamma_{\text{mol}}}{2}} \right)\psi_g
    \label{psi e dans psi g}
\end{equation}
The secondary term on the right-hand side acts as a complex, energy-dependent optical potential $V_{\text{opt}}$. In quantum scattering theory, the modification of the scattering length $\Delta a$ is directly driven by the projection of this optical potential onto the ground state wavefunction $\psi_g$. Near the molecular resonance, where the relative kinetic energy satisfies $E - \left(-\frac{\hbar^2\nabla^2}{2\mu} + V_e\right) \approx 0$, the integration of this localized coupling yields the unities-normalized complex scattering length originally formulated by Bohn and Julienne \cite{bohn1997_prospects}:
\begin{equation}
    a(\Delta) = a_{\text{bg}} + \frac{l_{\text{opt}}\Gamma_{\text{mol}}}{\Delta + i\frac{\eta\Gamma_{\text{mol}}}{2}}
    \label{a(delta)}
\end{equation}
where $a_{\text{bg}}$ is the real background scattering length representing native elastic collisions, and $\eta$ is a dimensionless phenomenological parameter introduced to account for additional technical broadening or loss channels observed experimentally \cite{yan_controlling_2013}. The parameter $l_{\text{opt}}$ is the temperature-independent optical scattering length, which encapsulates the strength of the light-induced interaction and scales strictly linearly with the laser intensity $I$. At low temperatures, it can be directly related to the stimulated linewidth $\Gamma_{\text{stim}}(k)$ through the Wigner law scaling \cite{tiesinga2005measurement}:
\begin{equation}
...
\begin{equation}
    a(\Delta) = a_{\text{bg}} + \frac{l_{\text{opt}}\Gamma_{\text{mol}}\Delta}{\Delta^2 + \left(\frac{\eta \Gamma_{\text{mol}}}{2}\right)^2} - i \frac{l_{\text{opt}}\Gamma_{\text{mol}}\frac{\eta\Gamma_{\text{mol}}}{2}}{\Delta^2 + \left(\frac{\eta \Gamma_{\text{mol}}}{2}\right)^2}
\end{equation}
