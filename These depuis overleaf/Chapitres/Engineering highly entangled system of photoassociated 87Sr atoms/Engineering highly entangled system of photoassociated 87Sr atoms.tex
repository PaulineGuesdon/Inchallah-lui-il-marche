...
\begin{equation}
    ...
    + \hat{V}(r)\right)\Psi_l = E \Psi_l
    \label{radial equation}
\end{equation}
- $\Psi_l$ the eigenfunction - by approximating the scattering of the atoms as a sum of an 
incoming plane wave with a spherical scattered one - considered as plane wave at long distance -

\begin{equation}
    \Psi_l = e^{i\mathbf{k}\cdot \mathbf{z}} + f(\theta) \frac{e^{ikr}}{r}
...
\begin{equation}
    \Psi_l = \frac{a_l}{r}sin(kr-\frac{l\pi}{2} + \delta_l)
\end{equation}
...
as $\Psi_l - e^{ikz}$ in a spherical basis where the solutions are a sum on all the partial waves.

\begin{equation}
    \Psi_l \simeq \frac{i}{2kr}\sum_{l=0}^{\infty}(2l+1)\left((-1)^l e^{-ikr} - S_l e^{ikr}\right)
\end{equation}
with $S_l = e^{2i\delta_l}$ the scattering amplitude.

The final elastic cross section that tells the surface area of the scattering is

\begin{equation}
    \sigma_{el} = \frac{\pi}{k^2}\sum_{l=0}^{\infty}(2l+1)|1-S_l|^2
\end{equation}

For the inelastic collisions cross section reads as
\begin{equation}
    \sigma_{inel} = \frac{\pi}{k^2}\sum_{l=0}^{\infty}(2l+1)(1-|S_l|^2)
    \label{inelastic cross section}
\end{equation}

The term $1-|S_l|^2$ quantifies the lost part of the incoming wave. 

Equation \ref{inelastic cross section} reveals that we need to introduce a new writing 
of $|S_l|^2$ with a complex term otherwise the inelastic cross section would be zero. Also
it has to be inferior to one for losses.\\
At $l=0$ 
...
A direct result of these equations is that we can write a general scattering length as
\begin{equation}
    a = a' + i l_{opt}
\end{equation}

$a$ is what we call the scattering length and is the radius of the scattering cross 
section $\sigma_e = 4\pi|a|^2$. $l_opt$ is the optical length, the radius of the 
inelastic scattering cross section $\sigma_{inel} = 4\pi/k l_{opt}$.\\ 
