...
Engineering Dicke state would makes possible to go beyond this limitation. For such system variance express as $\delta \phi \simeq \frac{1}{N}$
...
same for all the isotopes because we can validate the Born-Oppenheimer estimation for all isotopes. Then in the 
integral
...
only the reduced mass will change upon the isotope. Then with respect with the condition on the phase the energies $E$ 
...
complex (see figure \ref{fig:pa-ytterbium-potentials}) than many species that have been studied and for which the spectroscopy is well tested. It is indeed expected 
to count 205 different configurations of $(T, F, R)$ for the excited molecular state on intercombination line with an
atomic potential written as \ref{ytt potential} considering an hyperfine term. 
...
They have been capable of measuring around $80$ lines visible in figure \ref{173Yb spectro} that they could not identify to the associated molecular potential.
The atom loss plotted on the detuning of the PA indicates the position of the bounded states 
starting from the atomic resonance $\ket{^{1}S_0, F=5/2} \rightarrow \ket{^{3}P_1, F=7/2}$. 
Around this latter they do shine the atoms for $30$ms with really low intensity $I = 37$mW/cm² to avoided
power broadening of the transition so they can see the nearest bounded state from the dissociation
limit. Then while they are getting more red detuned they do increase this intensity and optimize
the contrast of the transition with the illumination time.\\

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Chapitres/Engineering highly entangled system of photoassociated 87Sr atoms/173Yb_spectro_lines.png}
    \caption{Caption}
    \label{173Yb spectro}
\end{figure}
