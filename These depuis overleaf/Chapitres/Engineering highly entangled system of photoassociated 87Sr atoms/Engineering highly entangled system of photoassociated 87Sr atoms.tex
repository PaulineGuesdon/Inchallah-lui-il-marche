...
In a Born-Oppenheimer approximation the hamiltonian of a two-atom system is
\begin{equation}
    \hat{V_{BO}} = -\frac{\hbar^2}{2\mu} \frac{d^2u}{dr^2} + \left(\frac{\hbar^2l(l+1)}{2\mu r^2} 
    + V(r)\right)u(r) = E u(r)
\end{equation}
$V(r) = \frac{-C_n}{r^n}$.\\

As in \cite{dalibard_interactions_nodate} we are going to change the basis of the solution 
with in different regions of the inter-atomic distance $r$.
At long distance the kinetic energy of the atoms - fisrt part of the hamiltonian - is dominant.
For intermediate distances the force that counts is the van der Waals because the electronic shell
start to overlap. 
At shorter distance it is the coulomb and van der Waals forces that are dominant.

The solution of the far distance potential is a superposition of incoming wave and a 
scattered one that acquire a phase $\delta_l$

\begin{equation}
    \psi(r) =  sin(kr - l\frac{\pi}{2}) + tan(\delta_l) cos(kr - l\frac{\pi}{2})
\end{equation}
at this position $r = a$ where the potential appears as a scattering boundary. This distance 
is called the scattering length. At low energies
...
$\Delta J = 0, \pm 1$ gives $\Delta T = 0, \pm 1$ with $T=0 \rightarrow T=0$ forbidden.\\
...
The BO potential do not count the $1/r^2$ potential because we are in the s-wave regime (l=0) 
so the electronic shell do not repulse the atoms at short distance. 
